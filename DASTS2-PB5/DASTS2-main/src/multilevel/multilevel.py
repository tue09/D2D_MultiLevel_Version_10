...
class Multilevel:
    ...
    def match_point(self, staff_path, drone_path, score_edge, match_percent, tau, tau_a, num_cus, nested_point_dict, nested_tau_dict, nested_tau_a_dict, c1):
        ...
        for i, trip in enumerate(staff_path):
            for j in range(len(trip) - 1):
                point = trip[j]
                next_point = trip[j + 1]
                element = {'edge': [point, next_point], 'tau': tau[point, next_point]}
                tau_edge.append(element)
        ...
        tau_edge = sorted(tau_edge, key= lambda x: x['tau'], reverse= False)
        print(tau_edge)
        for score_edge in tau_edge:
    ...
    def match_point_with_new_edge_score(self, staff_path, drone_path, score_edge, new_edge_score, match_percent, tau, tau_a, num_cus, nested_point_dict, nested_tau_dict, nested_tau_a_dict, c1):
        # # get highest score edges
        # for point in range(1, num_cus):
        #     for next_point in range(point + 1, num_cus + 1):
        #         edge = [point, next_point]
        #         score = score_edge[nested_point_dict[point][-1] - 1][nested_point_dict[next_point][0] - 1]
        #         element = {'edge': edge, 'score': score}
        #         sorted_score_edge.append(element)
        #         edge = [next_point, point]
        #         score = score_edge[nested_point_dict[next_point][-1] - 1][nested_point_dict[point][0] - 1]
        #         element = {'edge': edge, 'score': score}
        #         sorted_score_edge.append(element)  
        # sorted_score_edge = sorted(sorted_score_edge, key= lambda x: x['score'], reverse= False)
        ...
        next_level_nested = {}
        # get highest score edges
        # flatten new scored edge
        flatten_new_scored =  [item for sublist in new_edge_score for item in sublist]
        sorted_list = sorted(flatten_new_scored, reverse=True)
        # get 20% maximum of new score edge
        index_20_percent = int(num_cus * 0.2)
        max_20_percent = sorted_list[index_20_percent]
        tau_edge = []
        #
        for i, trip in enumerate(staff_path):
            for j in range(len(trip) - 1):
        ...
        for score_edge in tau_edge:
            edge = score_edge['edge']
            new_score = new_edge_score[edge[0]][edge[1]]
            if edge[0] not in point_will_match and edge[1] not in point_will_match and new_score < max_20_percent:
                point_will_match.append(edge[0])
                point_will_match.append(edge[1])
            if len(point_will_match) >= num_cus_match*2:
                break
    ...
    def run_multilevel(self, verbose= True):
        ...
        print("cur: ")
        print(cur)
        self.exist_score_edge = np.zeros((self.num_cus + 1, self.num_cus + 1), int)
        for level in range(self.num_level):
            ...
            result_by_iter = {}
            for iter in range(self.config.multilevel_params["tabu_max_iter"]):
                # variables'
                            ...
                            for point_idx in range(len_trip):
                                ...
                                for next_point_idx in range(len_trip):
                                    if point_idx != next_point_idx:
                                        ...
                                        if fitness - best_fitness < (-1)*0.000001 and fitness_info[1] == 0 and fitness_info[2] == 0:
                ...
                if cur != local_best_solution:
                    ...
                    self.update_exist_score_edge(self.get_ori_solution(cur[self.num_drone:], cur[:self.num_drone], nested_point_dict[level]))
                ...
                # result by iter
                one_result_by_iter = {}
                one_result_by_iter['act'] = act
                one_result_by_iter['ori_sol'] = str(self.get_ori_solution(cur[self.config.params["num_drone"]:], cur[:self.config.params["num_drone"]], nested_point_dict[level]))
                result_by_iter[iter] = one_result_by_iter
                # remove tabu 
                idx = 0
                idx_1 = 0
            ...
            if level == 0:
                ...
                rs_match = self.match_point_with_new_edge_score(best_solution[self.config.params["num_drone"]:], best_solution[:self.config.params["num_drone"]], score_edge, self.exist_score_edge,self.match_percent, self.inp["tau"], self.inp["tau_a"],
                                            self.num_cus, nested_point_dict[level], nested_tau_dict[level], nested_tau_a_dict[level], self.pure_inp["C1"])
    ...
    def update_exist_score_edge(self, solution):
        staff_path_list = solution[self.pure_config.params.num_drone:]
        drone_path_list = solution[:self.pure_config.params.num_drone]
        for i, staff in enumerate(staff_path_list):
            ...
            for j in range(len(staff) - 1):
                ...
                self.exist_score_edge[found_point][next_point] += 1
